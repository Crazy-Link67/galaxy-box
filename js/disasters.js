...
class NukeMissile {
    ...
    update(world, entityManager, disasterManager, particleSystem, audio) {
        ...
        // Smoke and rocket flame trail
        if (particleSystem) {
            particleSystem.spawn(this.x, this.y - this.vy * 0.5, -this.vx * 0.2, -this.vy * 0.3, 2.5, '#f97316', 20, 'fire');
            particleSystem.spawn(this.x, this.y - this.vy * 0.5, (Math.random() - 0.5) * 0.5, -0.4, 4, '#6b7280', 30, 'smoke');
        }

        if (this.y >= this.targetY || Math.hypot(this.targetX - this.x, this.targetY - this.y) < 8) {
            this.active = false;
